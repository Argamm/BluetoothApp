package com.zdravnica.app.navigation.app.models

internal object AppDestinationsRouteNames {
    const val ROOT = "Route"
    const val CONNECTION = "Connection_page"
    const val SHOW_DEVICE_DIALOG = "Show bluetooth devices dialog"
    const val SHOW_CANCEL_PROCEDURE_DIALOG = "Cancel Procedure Dialog"
    const val MAIN = "Main_page"
    const val SELECT_PROCEDURE_SCREEN = "SelectProcedureScreen"
    const val PROCEDURE_SCREEN = "ProcedureScreen"
    const val MENU_SCREEN = "MenuScreen"
    const val PREPARING_THE_CABIN_SCREEN = "PreparingTheCabinScreen"
    const val PROCEDURE_PROCESS_SCREEN = "ProcedureProcessScreen"
}